describe("Planet", function() {
  ...
  describe("eliptical orbit", function() {
    it("will have a variable velocity based on its location in orbit", function() {
      expect(duna.getVelocity(0)).toBeCloseTo(7907, 0)
      expect(duna.getVelocity(17315400 / 4)).toBeCloseTo(7586, 0)
      expect(duna.getVelocity(17315400 / 2)).toBeCloseTo(7154, 0)
      expect(duna.getVelocity(17315400 * 3 / 4)).toBeCloseTo(7586, 0)
    })
    ...
    it("has a prograde that is changes based on the location in the orbit", function() {
      ...
      expect(duna.getCartesianPrograde(0)).toBeCloseTo(-Math.PI / 2, 1)
      expect(duna.getCartesianPrograde(p / 2)).toBeCloseTo(Math.PI / 2, 1)
      expect(duna.getCartesianPrograde(p)).toBeCloseTo(-Math.PI / 2, 1)
      expect(duna.getCartesianPrograde(p / 4)).toBeCloseTo(Math.TAU, 0)
    })
